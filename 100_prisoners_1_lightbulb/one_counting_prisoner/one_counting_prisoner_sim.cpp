...
int num_days_for_new_prisoner(std::vector<bool>& counted_prisoners) {
    int num_days = 1;
    while (true) {
        int chosen_prisoner = random_prisoner();
...
int main() {
    ...
    simulated_results.reserve(NUM_ITERATIONS);
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        int counting_prisoner = random_prisoner();
        ...
        simulated_results.push_back(num_days_for_escape);
    }
    ...
    auto [min, max] = std::minmax_element(simulated_results.begin(), simulated_results.end());
